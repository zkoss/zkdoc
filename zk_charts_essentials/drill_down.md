...
The main benefit of implementing drill-down manually is flexibility:
this approach can show multiple series after drilling down. The trade-off
is that you have to handle everything yourself, including UI updates and
data loading.
...
public class DisplayMultipleSeriesComposer extends SelectorComposer<Component> {
    ...
    private static final String DRILLDOWN_LEVEL = "LEVEL";
...
- We store the drill-down level as an attribute of the *Charts*
  component. You can store any information to indicate the drill-down
  status.
...
When users click the drill-up button, we reload back to the original
data and change the button's visibility and cursor's style.
