...
**Vaadin is the smallest, and ZK is 19.5% above it — 652 lines against 779.** Most of that 127-line difference is structural: Vaadin composes its UI in Java with no template language, so where ZK writes a ViewModel and a ZUL template per view, Vaadin writes a single Java class. Vaadin also inherits Vaadin's Lumo theme, adding only six `getStyle()` chains, where the other five hand-write the design in CSS. That styling work is excluded from every total by the rule above, so it does not show up in these figures — but it does mean Vaadin's 652 lines buy a themed look rather than the custom one the others build.
