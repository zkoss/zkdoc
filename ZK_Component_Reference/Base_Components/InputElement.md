...
If you need to display an error message box that moves with scrolling, you can use [the `data-scrollable` attribute](http://books.zkoss.org/wiki/ZUML_Reference/ZUML/Namespaces/Client_Attribute/Data-Scrollable). This attribute allows the error message box to remain visible even when the page is scrolled.
