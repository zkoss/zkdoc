...
| Image | Description |
|-------|-------------|
| ![Zk css dom](images/Zk-css-dom.PNG) | -**outer**: the exterior of the specified component like splitter in vbox and hbox<br>-**header**: the header content, like grid, tree, listbox, and so on.<br>-**body**: the body content, like grid, tree, listbox, and so on.<br>-**inner**: the interior of the specified component, like slider and tab.<br>-**content**: like window's contentSclass or groupbox's contentSclass<br>-**footer**: describes the footer content, like grid, tree, listbox, and so on.<br>-**noheader**: no header element.<br>-**noborder**: no border element. |
